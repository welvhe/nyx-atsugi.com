...
<?php
  $cast_ranking_sql = "SELECT cast_rankings.*, ranking_categories.category, ranking_categories.title, casts.name AS cast_name, casts.post AS cast_post, casts.performance AS cast_performance, cast_images.image_url AS cast_image, shops.name AS shop_name, sns.main_sns AS main_sns FROM cast_rankings JOIN ranking_categories ON cast_rankings.ranking_category_id = ranking_categories.id JOIN casts ON cast_rankings.cast_id = casts.id LEFT JOIN cast_images ON casts.id = cast_images.cast_id AND cast_images.subject_id = 1 JOIN shops ON cast_rankings.subject_id = shops.id LEFT JOIN sns ON sns.subject_id = casts.id AND sns.subject_name = 'cast' AND sns.hosweb_privacy = 1 WHERE cast_rankings.subject_id = " . $shop_id . " AND cast_rankings.subject_name = 'shop' ORDER BY ranking_categories.priority ASC, cast_rankings.rank ASC";
  ...
  // 週間ランキング用SQL
  $weekly_access_ranking_sql = "SELECT casts.id AS cast_id, casts.name AS cast_name, casts.post AS cast_post, cast_images.image_url AS cast_image, shops.name AS shop_name, sns.main_sns AS main_sns, COUNT(access.id) AS access_count FROM access JOIN casts ON access.cast_id = casts.id LEFT JOIN cast_images ON casts.id = cast_images.cast_id AND cast_images.subject_id = 1 JOIN shops ON access.shop_id = shops.id LEFT JOIN sns ON sns.subject_id = casts.id AND sns.subject_name = 'cast' AND sns.hosweb_privacy = 1 WHERE access.shop_id = :shop_id AND access.created_at >= NOW() - INTERVAL 7 DAY GROUP BY casts.id ORDER BY access_count DESC";

  // 月間ランキング用SQL
  $monthly_access_ranking_sql = "SELECT casts.id AS cast_id, casts.name AS cast_name, casts.post AS cast_post, cast_images.image_url AS cast_image, shops.name AS shop_name, sns.main_sns AS main_sns, COUNT(access.id) AS access_count FROM access JOIN casts ON access.cast_id = casts.id LEFT JOIN cast_images ON casts.id = cast_images.cast_id AND cast_images.subject_id = 1 JOIN shops ON access.shop_id = shops.id LEFT JOIN sns ON sns.subject_id = casts.id AND sns.subject_name = 'cast' AND sns.hosweb_privacy = 1 WHERE access.shop_id = :shop_id AND access.created_at >= NOW() - INTERVAL 30 DAY GROUP BY casts.id ORDER BY access_count DESC";
      ...
      <?php if($cast_ranking_list) { ?>
        <section class="ranking">
          <div class="ranking__inner">
                    ...
                    <div class="rank_item">
                      <a href="<?= site_url('profile');?>/?cast=<?php echo $rank['cast_id'];?>">
                        ...
                        <div class="img_container">
                          <img src="<?= $rank['cast_image'] ? htmlspecialchars($rank['cast_image']) : wp_get_attachment_url( $no_img ); ?>" alt="<?= htmlspecialchars($rank['cast_name']); ?>">
                          <?php
                            ...
                            if($svg != '') {
                              echo '<span class="schedule__list-item-thumnail-snslink--'.$cast_thema_type.' sns_icon_bg_color"><i class="schedule__list-item-thumnail-tiktok-icon--'.$cast_thema_type.' sns_icon_color">'.$svg.'</i></span>';
                            }
                          ?>
                        </div>
            ...
            <a class="ranking__detail-link--<?php echo $button_shape_thema_type; ?> button_style border_color" href="<?= site_url('cast'); ?>/"><span>staff page</span></a>
          </div>
        </section>
      <?php } ?>
      <!-- オフィシャルランキングここまで -->
                  ...
                  <?php foreach ($weekly_ranking_list as $index => $rank) { ?>
                    <div class="rank_item">
                      <a href="<?= site_url('profile');?>/?cast=<?php echo $rank['cast_id'];?>">
                        ...
                        <p class="post"><?= htmlspecialchars($rank['cast_post'] ?: ''); ?></p>
                        <p class="cast_name"><?= htmlspecialchars($rank['cast_name']); ?></p>
                      </a>
                    </div>
                  <?php } ?>
...
"@type":"ListItem",
"position":2,
"item":{"name":"RANKING","@id":"<?= site_url('ranking');?>/"}
}
]
}
]
