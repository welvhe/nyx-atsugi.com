...
<?php if (empty($_SERVER['REQUEST_URI']) || $_SERVER['REQUEST_URI'] === '/') { ?>
  <script>
    $(document).ready(function() {
      // cast ranking
      <?php $sr_num = get_query_var('sr_num'); ?> // from cast-ranking.php
      const srMaxInitialDisplay = <?= $sr_num['sr_max_initial_display']; ?>;
      const srIncrementCount = <?= $sr_num['sr_increment_count']; ?>;

      let srTabState = {};

      $('.cast-tab-link:first').addClass('active');
      $('.cast-rank-content:first').show();
      $('.cast-ranking-title').text($('.cast-tab-link:first').data('title'));
      $('#sr-after-btn').hide();
      
      $('.cast-rank-content').each(function() {
      ...
      $('.cast-tab-link').on('click', function(e) {
        e.preventDefault();
        
        let index = $(this).data('index');
        let newTitle = $(this).data('title');

        $('.cast-tab-link').removeClass('active');
        $(this).addClass('active');

        $('.cast-rank-content').hide();
        $(`.cast-rank-content[data-index="${index}"]`).fadeIn(500);

        $('.cast-ranking-title').text(newTitle);
      ...
      $('#sr-view-more-btn').on('click', function(event) {
        ...
        let activeTab = $('.cast-rank-content:visible').data('index');
        let state = srTabState[activeTab];

        let hiddenItems = $(`.cast-rank-content[data-index="${activeTab}"] .rank_item.hidden-rank`);
      ...
      srCheckViewMoreVisibility($('.cast-rank-content:visible').data('index'));
...
<?php } else if (strpos($_SERVER['REQUEST_URI'], '/ranking') !== false) { ?>
  ...
  $(document).ready(function() {
    ...
    $('.cast-tab-link').click(function(e) {
      e.preventDefault();
      ...
      $('.cast-rank-content').hide();
      $('.cast-rank-content[data-index="' + index + '"]').show();
