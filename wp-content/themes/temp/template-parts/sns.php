<?php
  ...
  $sns_list_sql_shop = "SELECT sns_posts.`id` AS `id`, sns.`instagram_account_url` AS `instagram_account_url`, sns.`youtube_account_url` AS `youtube_account_url`, sns.`twitter_account_url` AS `twitter_account_url`, sns.`tiktok_account_url` AS `tiktok_account_url`, sns_posts.`parent_id` AS `parent_id`, sns_posts.`priority` AS `priority`, sns_posts.`subject_id` AS `subject_id`, sns_posts.`subject_name` AS `subject_name`, sns_posts.`sns_type` AS `sns_type`,sns_posts.`guid` AS `guid`, sns_posts.`original_image_url` AS `original_image_url`, sns_posts.`original_video_url` AS `original_video_url`,sns_posts.`image_url` AS `image_url`, sns_posts.`video_url` AS `video_url`, sns_posts.`description` AS `description`, sns_posts.`published_at` AS `published_at`, shops.`id` AS `shop_id`, NULL AS `cast_id`,shops.`name` AS `name`,shops.`name` AS `shop_name`, shop_images.`image_url` AS `thumbnail` FROM `sns_posts` JOIN `sns` ON sns.`subject_id` = sns_posts.`subject_id` AND sns.`subject_name` = sns_posts.`subject_name` JOIN `shops` ON shops.`id` = sns_posts.`subject_id` JOIN `shop_images` ON shop_images.`shop_id` = shops.`id` WHERE shop_images.`images_subject` = 'サムネ' AND shops.`id` = " . $shop_id . " AND sns.`hosweb_privacy` = 1 AND ($sns_type_condition_shop)  AND (sns_posts.image_url IS NOT NULL OR sns_posts.image_url != '') ORDER BY `published_at` DESC LIMIT 100";
