<?php
  ...
  $casts_regular_sql = "SELECT * FROM casts JOIN cast_images ON casts.id = cast_images.cast_id WHERE cast_images.subject_id = 1 AND casts.shop_id = ".$shop_id." AND casts.hosweb_privacy = 1 ORDER BY casts.priority ASC Limit 8";
  $casts_regular = $pdo->prepare($casts_regular_sql);
  $casts_regular->execute();

  if(($casts_count['count(id)'] > 0) && $change_content == "cast") {
?>

<section class="schedule">
  <div class="schedule__inner--<?php echo $cast_thema_type; ?>">
    <h2 class="schedule__title-main--<?php echo $cast_thema_type; ?> title_style">S T A F F</h2>
    <h3 class="schedule__title-sub--<?php echo $cast_thema_type; ?> sub_title_style">スタッフ一覧</h3>
    <ul class="schedule__list--<?php echo $cast_thema_type; ?>">
      <?php
        foreach($casts_regular as $cast) {
          ...
          $sns = $sns->fetch();
      ?>
          <li class="schedule__list-item--<?php echo $cast_thema_type; ?> border_color">
            <a href="<?= site_url('profile');?>/?cast=<?php echo $cast['cast_id'];?>">
              <div class="schedule__list-item-thumnail--<?php echo $cast_thema_type; ?>">
                <?php
                  if($cast['image_url'] != "") {
                    $cast['image_url'] = str_replace('http://', 'https://', $cast['image_url']);
                ?>
                    <img class="schedule__list-item-thumnail-img--<?php echo $cast_thema_type; ?>" src="<?php echo $cast['image_url'];?>" alt="<?php echo $store_name; ?>所属スタッフの「<?php echo $cast['name'];?>」の肖像写真" />
                <?php
                  } else {
                ?>
                    <img class="schedule__list-item-thumnail-img--<?php echo $cast_thema_type; ?>" src="<?php echo wp_get_attachment_url( $no_img );  ?>" alt="画像未登録時の代替え画像の<?php echo $store_name; ?>のロゴバナー" />
                <?php
                  }
              ...
              <div class="schedule__list-item-content--<?php echo $cast_thema_type; ?> schedule_cast_name_bg_color">
                <p class="schedule__list-item-content-title--<?php echo $cast_thema_type; ?> schedule_cast_name_text_color"><i><?php echo $cast['post'];?></i></p>
                <h4 class="schedule__list-item-content-title--<?php echo $cast_thema_type; ?> schedule_cast_name_text_color"><i><?php echo $cast['name'];?></i></h4>
              </div>
            </a>
          </li>
      <?php
    ...
    <a href="<?= site_url('cast'); ?>/" class="schedule__arcives-link--<?php echo $button_shape_thema_type; ?> button_style border_color"><span>staff page</span></a>
  </div>
</section>
